...
export default function FinanceDashboard() {
  ...
  // --- Dynamic Base Date Logic for Cards ---
  const baseDate = new Date(filterYear, filterMonth - 1, 1);

  // 1. Selected Month Stats
  const selectedMonthStats = calculateStats(filteredData);

  // 2. Previous Month (relative to selection)
  const prevMonthDate = new Date(baseDate);
  prevMonthDate.setMonth(prevMonthDate.getMonth() - 1);
  const previousMonthStats = calculateStats(transactions.filter(t => {
    const d = new Date(t.date);
    return d.getMonth() === prevMonthDate.getMonth() && d.getFullYear() === prevMonthDate.getFullYear();
  }));

  // 3. Last 3 Months (relative to selection)
  const threeMonthsAgoLimit = new Date(baseDate);
  threeMonthsAgoLimit.setMonth(threeMonthsAgoLimit.getMonth() - 2);
  const rangeStats = calculateStats(transactions.filter(t => {
    const d = new Date(t.date);
    return d >= threeMonthsAgoLimit && d <= new Date(filterYear, filterMonth, 0);
  }));

  // 4. Life Time Stats (Always same)
  ...
  const StatCard = ({ title, income, expense, net, icon: Icon, color }) => (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        ...
        <div className={`p-2 rounded-xl bg-zinc-900 border border-zinc-800 text-${color === 'blue' ? 'blue-400' : color === 'emerald' ? 'emerald-400' : color === 'purple' ? 'purple-400' : 'amber-400'}`}>
          <Icon size={16} />
        </div>
      </div>
  ...
  return (
    <div className=" min-h-screen text-zinc-200  font-sans">
      ...
      {/* Cards are now dynamic based on selection */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        <StatCard title="Selected Month" {...selectedMonthStats} icon={Calendar} color="emerald" />
        <StatCard title="Month Before" {...previousMonthStats} icon={HistoryIcon} color="blue" />
        <StatCard title="Selected 3-Month Window" {...rangeStats} icon={CalendarRange} color="purple" />
        <StatCard title="Life Time Total" {...lifeTimeStats} icon={Wallet} color="amber" />
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-8">
          {/* Monthly Summary Table (Jan to Dec) */}
          <div className="bg-[#0d121f] border border-zinc-800/50 rounded-[2.5rem] overflow-hidden shadow-2xl">
            <div className="px-8 py-6 border-b border-zinc-800/50 flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold text-white">Annual Growth Summary</h2>
            </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-zinc-800/50">
                  {loading ? (
                    ...
                    <tr><td colSpan="4" className="py-12 text-center text-zinc-600 italic">No records for selection</td></tr>
                  ) : (
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-[2.5rem] p-8 text-white shadow-xl">
            ...
            <p className="text-emerald-100 text-sm leading-relaxed mb-6 opacity-90">
               Based on your filter, top expense is <span className="font-bold underline text-white">{filteredData.filter(t => t.type === 'expense').sort((a,b) => b.amount - a.amount)[0]?.category || 'N/A'}</span>.
            </p>
            <div className="bg-white/10 rounded-2xl p-4 backdrop-blur-sm border border-white/10">
              <p className="text-[10px] uppercase font-bold opacity-70 mb-1">Monthly Budget Goal</p>
              <div className="h-2 w-full bg-black/20 rounded-full overflow-hidden">
                <div className="h-full bg-white rounded-full w-[45%]" />
              </div>
              <p className="text-right text-[10px] mt-2 font-bold">45% Used</p>
            </div>
          </div>
        </div>
      </div>
